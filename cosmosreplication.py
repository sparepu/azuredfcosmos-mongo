...
def getOptions(args=sys.argv[1:]):
    ...
    parser.add_argument("-sinksecret", "--secretnameforsinkconnectionstring", help="Secret Name for sink connection string", required=True)
    parser.add_argument("-sourcedb", "--sourcedatabasename", help="Database Name in source cosmos", required=True)
    parser.add_argument("-sinkdb", "--sinkdatabasename", help="Database Name for sink cosmos", required=True)
    parser.add_argument("-sourcecollection", "--sourcecollectionname", help="Collection Name in source cosmos", required=True)
    parser.add_argument("-sinkcollection", "--sinkcollectionname", help="Collection Name in sink cosmos", required=True)
...
def datafactory(sourceconnectionstring, sinkconnectionstring):
    ...
    sourcedataset = CosmosDbMongoDbApiCollectionDataset(linked_service_name=source_ds_ls, collection= collection, schema=None, type='Json')
    ...
    if options.integrationruntime is not None:
       sink_ls_azure_cosmos = CosmosDbMongoDbApiLinkedService(connection_string=sinkconnectionstring, database=options.sinkdatabasename, connect_via=integrationruntime)
    else:
       sink_ls_azure_cosmos = CosmosDbMongoDbApiLinkedService(connection_string=sinkconnectionstring, database=options.sinkdatabasename)
    sink_ls = adf_client.linked_services.create_or_update(rg_name, df_name, sink_ls_name, sink_ls_azure_cosmos)
    print_item(sink_ls)

    sink_ds_name = 'sinkDS'
    sink_ds_ls = LinkedServiceReference(reference_name=sink_ls_name)
    collection = options.sinkcollectionname
    sinkdataset = CosmosDbMongoDbApiCollectionDataset(linked_service_name=sink_ds_ls, collection= collection, schema=None, type='Json')
    ...
    cosmos_sink = CosmosDbMongoDbApiSink(write_behavior='upsert')
    dsin_ref = DatasetReference(reference_name=source_ds_name)
    dsOut_ref = DatasetReference(reference_name=sink_ds_name)
    copy_activity = CopyActivity(name=act_name,inputs=[dsin_ref], outputs=[dsOut_ref], source=cosmos_source, sink=cosmos_sink)
